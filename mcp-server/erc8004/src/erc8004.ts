...
function loadAbi(abiPath: string) {
  ...
  if (Array.isArray(json)) return json;     // ← いま手元の形式（配列）
  if (Array.isArray(json?.abi)) return json.abi; // ← artifacts 形式
  throw new Error(`Invalid ABI JSON (no array found): ${abiPath}`);
}

type WalletRole = 'ANALYST' | 'VALIDATOR' | 'CLIENT';

export class ERC8004Client {
  provider: JsonRpcProvider;
  wallets: Record<WalletRole, Wallet>;
  identity: Contract;
  reputation: Contract;
  validation: Contract;
  ...
  // ---- Helpers ----
  wallet(role?: WalletRole) {
    const key = (process.env.DEFAULT_WALLET as WalletRole) || 'VALIDATOR';
    return this.wallets[role ?? key];
  }

  // ---- Identity ----
  async registerAgent(domain: string, role: WalletRole) {
  ...
  // ---- Reputation ----
  async authorizeFeedback(clientId: number, serverId: number, role: WalletRole) {
  ...
  // ---- Validation ----
  async requestValidation(validatorId: number, serverId: number, dataHashHex: string, role: WalletRole) {
    const w = this.wallet(role); // server の本人鍵
  ...
  async submitValidationResponse(dataHashHex: string, score: number, role: WalletRole) {
  ...
  async getValidationResponse(dataHashHex: string) {
    const fn = this.validation.getFunction('getValidationResponse');
    const [hasResponse, response] = await fn('0x' + dataHashHex) as [boolean, bigint];
