...
export function saveAnalysis(hashHex: string, obj: any) {
  ensureDirs();
...
export function loadAnalysis(hashHex: string): any | null {
  const p = path.join(dataDir, `${hashHex}.json`);
  if (!fs.existsSync(p)) return null;
  return JSON.parse(fs.readFileSync(p, 'utf8'));
}

export function saveValidation(hashHex: string, obj: any) {
  ensureDirs();
